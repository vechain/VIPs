...
A) The *sender endpoint* initiates a request. The request includes a sender address and a full transaction body. The request is sent to the *authorization endpoint* (in the form of a predefined URI).
...
| name | description |
| --- | --- |
| sender   | String. The origin of the transaction, address type.|
| body | String. JSON string of the full transaction body. |
...
{
    "sender": "0xDe9daCD226e1CE48036c87Cfd956bf0229d2475d",
    "body": (see above "body")
}
```

B) The *authorization endpoint* authenticates the caller, then inspects the content of the sender address and transaction body. It establishes whether to grant or deny the signing request.
...
| name | description |
| --- | --- |
| payer   | String. The gas payer of the transaction, address type.|
| payerSignature | String. 65 bytes of signature, in hex format (130 chars). |
...
Pragma: no-cache
{
    "payer": "0xB780dcdb34c239e7Cc55C5aCEaccd31588FdC99f",
    "payerSignature": "f8fe82c74f9e1f5bf443f8a7f8eb968140f554968fdcab0a6ffe904e451c8b9244be44bccb1feb34dd20d9d8943f8c131227e55861736907b02d32c06b934d7200"
}
```

If an error occurs, the authorization endpoint responds with an **HTTP 400** (Bad Request) status code and includes the following parameters:

**Error Params**
| field | value | description |
| --- | --- | --- |
| error| invalid_request | Missing parameters or type error of values, or otherwise malformed. |
| | unauthorized_client | The sender endpoint is not authorized to use this service. |
| | unauthorized_grant | The sender (origin of the tx) is not qualified to use this service. |
...
Pragma: no-cache
{
    ...
    "description": "Missing body.gas field."
}
```

The service can extend "error" codes for other errors.
...
### Unauthorized Access
As the server is exposed online and accessed publicly, measures shall be taken to harden the service such as limiting visiting certain sender endpoints. The service developer can utilize Oauth 2.0 to discipline the senders. For example a token in the HTTP header section:
...
### Offline of Service
This happens when the server is down. It is recommended that the server provides a reasonable timeout and HTTP error code for **sender endpoint** to try again later.
