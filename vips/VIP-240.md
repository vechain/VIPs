...
>
> With the implementation of the Hayabusa upgrade ([VIP-254](./VIP-254.md)), holding VET no longer generates VTHO. As a result, the recommended processes and staking model described in this VIP are **no longer applicable** and should not be used for new implementations.
...
> **Note:** The mechanism described below was valid prior to the Hayabusa upgrade (VIP-254). After VIP-254 implementation, VET no longer generates VTHO automatically.
...
VTHO was generated automatically for every account with a positive VET balance. The incremental of VTHO was solely depending on two facts: the amount of VET in the account and the duration of time.
